...
import {
    ...
    RiHashtag, RiInformationLine, RiGalleryLine, RiDeleteBinLine,
    RiStarLine, RiRocketLine, RiCheckLine, RiAddLine, RiFilePdfLine,
    RiUserAddLine, RiHomeLine
} from 'react-icons/ri';
import Input from './Input';
import Button from './Button';
import { createProperty } from '../apis/property_api';
...
const AddProperties = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setFormData({
                ...
                community: '', location: '', emirate: 'Dubai', country: 'UAE',
                property_category: 'Apartment', property_types: [],
                starting_price: '', currency: 'AED', handover: '',
                featured: false, is_new: false, status: 'active',
                amenities: [], units: [], agents: [], nearby_locations: [],
                payment_plan: []
            });
            setHeroImage(null);
            setHeroUrl('');
            ...
            setError(typeof err === 'string' ? err : 'Failed to create property listing. Please check required fields.');
            console.error(err);
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="p-6 md:p-8 max-w-5xl mx-auto animate-fade-in text-left">
            <div className="mb-10">
                <h1 className="text-3xl font-bold text-white mb-2">List New <span className="text-[#BD9B5F]">Property</span></h1>
                <p className="text-gray-400">Expand the portfolio with exclusive listings across the Emirates.</p>
            </div>
            ...
            <AnimatePresence>
                {success && (
                    <motion.div
                        ...
                        className="mb-8 p-4 bg-green-500/10 border border-green-500/20 rounded-2xl text-green-400 text-sm flex items-center gap-3 overflow-hidden"
                    >
                        <RiCheckLine size={24} className="flex-shrink-0" />
                        <div>
                            <p className="font-bold">Listing Published!</p>
                            <p className="text-xs opacity-80">The property is now live and visible to agents and clients.</p>
                        </div>
                    </motion.div>
                )}
            </AnimatePresence>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
                {/* Visual Header / Hero Settings */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <section className="md:col-span-2 space-y-8">
                        {/* Basic Info */}
                        <div className="bg-[#1a1a1a] p-8 rounded-3xl border border-[#333] space-y-6">
                            <div className="flex items-center gap-3 mb-2">
                                <RiInformationLine size={22} className="text-[#BD9B5F]" />
                                <h2 className="text-lg font-bold text-white uppercase tracking-wider">Basic Information</h2>
                            </div>
                            ...
                            <Input
                                ...
                                placeholder="e.g. Burj Azizi - Ultra Luxury Apartment"
                                required
                            />

                            <Input
                                label="Headline (Short Catchy Phrase)"
                                name="headline"
                                value={formData.headline}
                                onChange={handleChange}
                                placeholder="e.g. Ocean View Living at its Best"
                            />
                            ...
                            <div className="flex flex-col space-y-2">
                                ...
                                <textarea
                                    ...
                                    placeholder="Detailed description of the property, features, and neighborhood..."
                                    required
                                />
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <select
                                        ...
                                        onChange={handleChange}
                                        className="w-full bg-[#222] border border-[#333] rounded-2xl px-5 py-4 text-white focus:border-[#BD9B5F] focus:outline-none transition-all appearance-none cursor-pointer"
                                    >
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                    ...
                                    {/* Manually added property types */}
                                    {formData.property_types.filter(t => !propertyTypeOptions.includes(t)).map(type => (
                                        <label
                                            key={type}
                                            className="flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all bg-[#BD9B5F]/10 border-[#BD9B5F] text-white shadow-lg shadow-[#BD9B5F]/5"
                                        >
                                            <input
                                                type="checkbox"
                                                className="hidden"
                                ...
                                {/* Manual Entry for Property Types */}
                                <div className="flex gap-2 mt-4">
                                    <input
                                        ...
                                        className="flex-1 bg-[#222] border border-[#333] rounded-xl px-4 py-2 text-xs text-white focus:border-[#BD9B5F] outline-none"
                                    />
                                    <button
                                        type="button"
                                        onClick={() => {
                                            if (customType.trim()) {
                                                if (!formData.property_types.includes(customType.trim())) {
                                                    handleArrayChange('property_types', customType.trim());
                                                }
                                                setCustomType('');
                                            }
                                        }}
                                        className="p-2 bg-[#BD9B5F] rounded-xl text-black hover:bg-[#a68950] transition-all"
                                    >
                                        <RiAddLine size={20} />
                                    </button>
                                </div>
                        ...
                        <div className="bg-[#1a1a1a] p-8 rounded-3xl border border-[#333] space-y-6">
                            <div className="flex items-center gap-3 mb-2">
                                <RiInformationLine size={22} className="text-[#BD9B5F]" />
                                <h2 className="text-lg font-bold text-white uppercase tracking-wider">Property Description</h2>
                            </div>
                            <textarea
                                name="description"
                                value={formData.description}
                                onChange={handleChange}
                                rows="6"
                                className="w-full bg-[#222] border border-[#333] rounded-2xl px-6 py-5 text-white focus:border-[#BD9B5F] focus:outline-none transition-all resize-none"
                                placeholder="Describe the property, its lifestyle, and key selling points..."
                                required
                            />
                        </div>
                        ...
                        <div className="bg-[#1a1a1a] p-8 rounded-3xl border border-[#333] space-y-6">
                            <div className="flex items-center gap-3 mb-2">
                                <RiCheckLine size={22} className="text-[#BD9B5F]" />
                                <h2 className="text-lg font-bold text-white uppercase tracking-wider">Amenities & Features</h2>
                            </div>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                ...
                                {/* Manually added amenities */}
                                {formData.amenities.filter(a => !amenityOptions.includes(a)).map(amenity => (
                                    <label
                                        key={amenity}
                                        className="flex items-center gap-3 p-4 rounded-2xl border cursor-pointer transition-all bg-[#BD9B5F]/10 border-[#BD9B5F] text-white shadow-lg shadow-[#BD9B5F]/5"
                                    >
                                        <input
                                            type="checkbox"
                                            className="hidden"
                                            ...
                                            onChange={() => handleArrayChange('amenities', amenity)}
                                        />
                                        <RiCheckLine className="text-[#BD9B5F]" />
                                        <span className="text-[10px] font-bold uppercase tracking-widest">{amenity}</span>
                                    </label>
                                ))}
                            </div>

                            {/* Manual Entry for Amenities */}
                            <div className="flex gap-2 mt-4">
                                <input
                                    type="text"
                                    value={customAmenity}
                                    ...
                                    onClick={() => {
                                        if (customAmenity.trim()) {
                                            if (!formData.amenities.includes(customAmenity.trim())) {
                                                handleArrayChange('amenities', customAmenity.trim());
                                            }
                                            setCustomAmenity('');
                                        }
                                    }}
                        ...
                        <div className="bg-[#1a1a1a] p-8 rounded-3xl border border-[#333] space-y-6">
                            <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-3">
                                    <RiHomeLine size={22} className="text-[#BD9B5F]" />
                                    <h2 className="text-lg font-bold text-white uppercase tracking-wider">Unit Configurations</h2>
                                </div>
                                <button
                                    type="button"
                                    onClick={addUnit}
                                ...
                                >
                                    <RiAddLine size={16} /> Add Unit
                                </button>
                            </div>

                            {formData.units.length === 0 ? (
                                <p className="text-gray-500 text-sm text-center py-8">No units added yet. Click "Add Unit" to create unit configurations.</p>
                            ) : (
                                <div className="space-y-4">
                                    {formData.units.map((unit, index) => (
                                        <div key={index} className="bg-[#222] p-6 rounded-2xl border border-[#333] space-y-4">
                                            <div className="flex items-center justify-between mb-2">
                                                <h3 className="text-sm font-bold text-[#BD9B5F] uppercase">Unit {index + 1}</h3>
                                                <button
                                                    type="button"
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                ...
                                                <Input
                                                    ...
                                                    type="number"
                                                    value={unit.sqm}
                                                    onChange={(e) => updateUnit(index, 'sqm', e.target.value)}
                                                    placeholder="120"
                                                />
                                                <Input
                                                    label="Area (sqft)"
                                            ...
                                            <div className="flex flex-col space-y-2">
                                                ...
                                                <textarea
                                                    ...
                                                    rows="2"
                                                    className="w-full bg-[#111] border border-[#333] rounded-xl px-4 py-3 text-white focus:border-[#BD9B5F] focus:outline-none transition-all resize-none text-sm"
                                                    placeholder="Unit-specific details, features, views..."
                                                />
                                            </div>
                                        </div>
                                    ))}
                        ...
                        <div className="bg-[#1a1a1a] p-8 rounded-3xl border border-[#333] space-y-6">
                            <div className="flex items-center gap-3 mb-2">
                                ...
                                <h2 className="text-lg font-bold text-white uppercase tracking-wider">Location & Address</h2>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    ...
                                    <select
                                        ...
                                        onChange={handleChange}
                                        className="w-full bg-[#222] border border-[#333] rounded-2xl px-5 py-4 text-white focus:border-[#BD9B5F] focus:outline-none transition-all"
                                        required
                                    >
                            ...
                            <Input
                                ...
                                onChange={handleChange}
                                placeholder="e.g. Palm Jumeirah, Andheri"
                            />

                            <Input
                                label="Full Address / Location Coordinates"
                                name="location"
                                value={formData.location}
                                onChange={handleChange}
                                placeholder="Detailed address or Map URL"
                            />
                        </div>
                        ...
                        <div className="bg-[#1a1a1a] p-8 rounded-3xl border border-[#333] space-y-6">
                            <div className="flex items-center gap-3 mb-2">
                                ...
                                <h2 className="text-lg font-bold text-white uppercase tracking-wider">Pricing & Timeline</h2>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <Input
                                    label={`Starting Price (${formData.currency})`}
                                    type="number"
                                    name="starting_price"
                    ...
                    <aside className="space-y-8">
                        ...
                        <div className="bg-[#1a1a1a] p-6 rounded-3xl border border-[#333] space-y-4">
                            <h3 className="text-sm font-bold text-white uppercase tracking-widest mb-4">Property Status</h3>
                            <div className="grid grid-cols-2 gap-3">
                                <label className={`flex items-center justify-center p-4 rounded-xl border-2 cursor-pointer transition-all ${formData.status === 'active' ? 'bg-green-500/10 border-green-500/50 text-green-400' : 'bg-[#222] border-[#333] text-gray-500'}`}>
                                    <input
                                        type="radio"
                                        name="status"
                                        value="active"
                                ...
                                <label className={`flex items-center justify-center p-4 rounded-xl border-2 cursor-pointer transition-all ${formData.status === 'inactive' ? 'bg-red-500/10 border-red-500/50 text-red-400' : 'bg-[#222] border-[#333] text-gray-500'}`}>
                                    <input
                                        type="radio"
                                        name="status"
                                        value="inactive"
                        ...
                        <div className="bg-[#1a1a1a] p-6 rounded-3xl border border-[#333] space-y-4">
                            <div className="flex items-center gap-2 mb-2">
                                <RiUserAddLine size={18} className="text-[#BD9B5F]" />
                                <h3 className="text-sm font-bold text-white uppercase tracking-widest">Assign Agents</h3>
                            </div>
                            <div className="max-h-48 overflow-y-auto space-y-2 custom-scrollbar">
                                {availableAgents.length === 0 ? (
                                    <p className="text-gray-500 text-xs text-center py-4">No agents available</p>
                                ) : (
                                    availableAgents.map(agent => (
                                        <label key={agent._id} className="flex items-center gap-3 p-3 bg-[#222] border border-[#333] rounded-xl cursor-pointer hover:border-[#BD9B5F]/50 transition-all">
                                            <input
                                                type="checkbox"
                                                checked={formData.agents.includes(agent._id)}
                                                onChange={() => handleArrayChange('agents', agent._id)}
                                                className="w-4 h-4 accent-[#BD9B5F]"
                                            />
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <p className="text-xs text-white font-medium truncate">{agent.agent_name || 'Agent'}</p>
                                                <p className="text-[10px] text-gray-500 truncate">{agent.agent_role || 'Sales Agent'}</p>
                                            </div>
                                        </label>
                                    ))
                                )}
                        ...
                        <div className="bg-[#1a1a1a] p-6 rounded-3xl border border-[#333] space-y-4">
                            <div className="flex items-center gap-2 mb-2">
                                <RiFilePdfLine size={18} className="text-[#BD9B5F]" />
                                <h3 className="text-sm font-bold text-white uppercase tracking-widest">Brochures (PDFs)</h3>
                            </div>
                            ...
                            <div className="bg-[#1a1a1a] p-6 rounded-3xl border border-[#333] space-y-4">
                                <div className="flex items-center justify-between mb-2">
                                    <div className="flex items-center gap-2">
                                        <RiFilePdfLine size={18} className="text-[#BD9B5F]" />
                                        <h3 className="text-sm font-bold text-white uppercase tracking-widest">Brochures</h3>
                                    </div>
                                    <button
                                        type="button"
                                ...
                                <label
                                    ...
                                    className="flex flex-col items-center justify-center p-4 bg-black/20 border border-[#333] rounded-xl cursor-pointer hover:border-[#BD9B5F]/50 transition-all text-gray-500 hover:text-[#BD9B5F]"
                                >
                                    <RiFilePdfLine size={24} className="mb-2" />
                                ...
                                {brochureItems.length > 0 && (
                                    <div className="space-y-3 mt-4">
                                        {brochureItems.map((item) => (
                                            <div key={item.id} className="bg-[#222] p-3 rounded-xl border border-[#333] space-y-2">
                                                <div className="flex items-center justify-between">
                                                    <RiFilePdfLine className={item.type === 'file' ? 'text-red-400' : 'text-[#BD9B5F]'} size={16} />
                                                    <button
                                                        type="button"
                        ...
                        <div className="bg-[#BD9B5F]/5 p-6 rounded-3xl border border-[#BD9B5F]/20 space-y-4">
                            <h3 className="text-sm font-bold text-[#BD9B5F] uppercase tracking-widest mb-4">Listing Status</h3>
                            <label className="flex items-center gap-4 p-3 bg-black/20 rounded-2xl cursor-pointer hover:bg-black/30 transition-all border border-transparent hover:border-[#BD9B5F]/20">
                                <input
                                    type="checkbox"
                                    name="featured"
                                    checked={formData.featured}
                                    onChange={handleChange}
                                    className="w-5 h-5 accent-[#BD9B5F] bg-transparent border-[#333]"
                                />
                                <div className="flex items-center gap-2">
                                    <RiStarLine className="text-[#BD9B5F]" />
                                    <span className="text-sm text-white font-medium">Featured Listing</span>
                                </div>
                            </label>
                            <label className="flex items-center gap-4 p-3 bg-black/20 rounded-2xl cursor-pointer hover:bg-black/30 transition-all border border-transparent hover:border-[#BD9B5F]/20">
                                <input
                                ...
                                />
                                <div className="flex items-center gap-2">
                                    <RiRocketLine className="text-[#BD9B5F]" />
                                    <span className="text-sm text-white font-medium">Mark as New Launch</span>
                                </div>
                            </label>
                        </div>
                        ...
                        <div className="bg-[#1a1a1a] p-6 rounded-3xl border border-[#333] space-y-4">
                            ...
                            <div className="space-y-4 mt-4">
                                ...
                                {/* Image Grid (Both files and URLs with valid values) */}
                ...
                <div className="pt-10 flex flex-col md:flex-row justify-end gap-4">
                    <button
                        ...
                        className="px-10 py-5 rounded-2xl border border-[#333] text-gray-400 font-bold hover:text-white transition-all uppercase tracking-widest text-xs"
                    >
                        Back to List
                    </button>
                    <Button
                        type="submit"
                        isLoading={loading}
                        className="px-12 py-5 shadow-[0_10px_30px_rgba(189,155,95,0.2)]"
                    >
                        Publish Listing
                    </Button>
                </div>
